...
$sql = "SELECT orderId, userName, dates, item FROM orders where userName='user'";
$result = $conn->query($sql);

if ($result->num_rows > 0) {
    echo "<table>
	<tr><td><a href='userloggedin.php'>Back</a></td></tr>
	<tr><th>Order Id</th><th>User Name</th><th>Dates</th><th>Items</th></tr>";
    // output data of each row
    while($row = $result->fetch_assoc()) {
        echo "<tr><td>".$row["orderId"]."</td><td>".$row["userName"]."</td><td>".$row["dates"]."</td><td>".$row["item"]."</td></tr>";
    }
    echo "</table>";
} else {
    echo "0 results";
